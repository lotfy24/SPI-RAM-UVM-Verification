module SPI_gld (clk,rst_n,SS_n,MOSI,tx_valid,tx_data,rx_valid_gld,MISO_gld,rx_data_gld);
...
always @(posedge clk) begin
    if(~rst_n)begin
        ...
        case (cs)
            IDLE:begin
                rx_valid_gld <=1'b0; // communication is off no ready data
            end
            CHK_CMD:begin
                counter <= 10;       // start of the counter because data needs 10 cycles to be in parallel form
                rx_valid_gld <=1'b0; // data is not ready 
            end
            WRITE:begin
                if(counter > 0)begin
                    rx_valid_gld <=1'b0;
            ...
            READ_DATA:begin
                if(tx_valid == 1'b1)begin
                    rx_valid_gld <=1'b0;    // send data to the master throgh MISO
                    if(counter > 0)begin
